...
transmit(canch,message_speed_request);%--------speed transmision requested every 100 ms---------

message_current_request = canMessage(bamocar_r,false,3);
message_current_request.Data = ([0x3d 0x5f 0x64]);
transmit(canch,message_current_request);%--------current transmision requested every 100 ms---------
...
hSlider1 = controllib.widget.Slider(hFig, [50,20,900,200], -100:5:100);
...
while get
    ...
    speed_conv = int16(floor(hSlider1.Value*32767/100/20));
    ...
    for i=1:length(REC)
        if REC(i).ID == bamocar_t
            switch REC(i).Data(1)
                case 0xa8%------filtered actual speed------------
                    ...
                    speed_act = int16(single(speed_act_conv)*vitesse_max/32767);
                    
                case 0x5f%------filtered actual current----------
                    byte2 = dec2hex(REC(i).Data(2));
                    if length(byte2) ~= 2
                        byte2 = strcat('0',byte2);
                    end
                    ...
                    current_act = single(current_act_conv)*1.5*I_max/32767;
...
transmit(canch,message_speed_request);%--------speed transmision stopped---------

message_current_request = canMessage(bamocar_r,false,3);
message_current_request.Data = ([0x3d 0x5f 0xff]);
transmit(canch,message_current_request);%--------current transmision stopped---------
